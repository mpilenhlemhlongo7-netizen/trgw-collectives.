...
- **Premium Publishing Package** (`js/store-config.js` → `packages`): a real, confirmed R15,000 fixed-price full-service publishing offer (editing, cover design, ISBN & print coordination, launch marketing), sold through the same cart/checkout as the books rather than the quote-only services. Featured on `shop.html` as its own elevated pricing card between the book grid and the quote-only Services section.
- **Payment — live today**: direct-to-Capitec. Melokuhle's real account (Mr. Melokuhle M. Mhlongo, Capitec Savings, acc. 2368365415, branch 470010) is in `js/store-config.js` and shown on the order confirmation screen along with the reference to quote. No merchant sign-up, no KYC wait — this works the moment the site is live. Instant Capitec Pay is also live via his real cellphone number (067 304 1992).
...
A Money & Mindset Movement booking page, built entirely from verified engagement Sibani already has — no invented workshop dates, group sizes, or pricing. Three tracks (Schools & TVET, Corporate & Institutional, Community Groups) map directly to real proof points already established elsewhere on the site (TVET Amajuba, BMW, Audi Park), a curriculum section reuses the real Five Pocket Wealth System breakdown, and every "Enquire to Book" link routes into the existing contact form pre-set to the Speaking & School Programmes category — the same enquiry-deep-link pattern used by the Shop's service cards.
